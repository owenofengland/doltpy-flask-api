...
@api.route("/<account>/<repo>/<branch>/tables/<arg>", methods=["GET"])
def get_allTables(account, repo, branch, arg):
    fullRepo = account + "/" + repo
    if arg == "all":
        return Response(getAllTables(fullRepo, branch), mimetype="application/json"), 200
    elif arg == "base":
        return Response(getBaseTables(fullRepo, branch), mimetype="application/json"), 200
    elif arg == "system":
        return Response(getSystemTables(fullRepo, branch), mimetype="application/json"), 200
    else:
...
@api.route("/<account>/<repo>/<branch>/select", methods=["GET"])
def get_selectQuery(account, repo, branch):
    fullRepo = account + "/" + repo
    requestJson = request.get_json()
    if requestJson['query']:
        return Response(getDataFromQuery(fullRepo, branch, requestJson['query']), mimetype="application/json"), 200
